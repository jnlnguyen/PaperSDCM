...
To generate our ML models for PL-based sample identifications, we choose a combination of supervised and unsupervised learning methods.
The following sections describe all our steps to generate these models. 
...
and the other contains details about the sample.
The latter acts as labels for the measurement, which are central for the evaluation of the classifier's performance.
...
\begin{description}
	...
	\item[\textbf{Origin}:] Name of the manufacturer or location. All retail samples have the same label.
	\item[\textbf{Color}:] Colour of the sample.
	\item[\textbf{isPlastic}:] Specifies if the sample material is a plastic or not.
	\item[\textbf{SampleID}:] Unique ID identifying the sample for each measurement.
	\item[\textbf{SessionID}:] Unique ID identifying the measurement session.
\end{description}
...
\begin{figure}[h]
	...
	\label{sfig:dataprep}
\end{figure}
We prepare our spectral data as illustrated in \autoref{sfig:dataprep}.
First, the spectral data and the corresponding background measurement are interpolated onto a common spectral axis.
The number of spectral bins is kept equal to the mean of number of bins in the overall set.
We then subtract the background measurement from the sample spectrum.
Once all spectra are processed in that manner, we concatenate the data into a single matrix.

Since we do not expect any signal below the laser peak, we estimate the offset from the baseline $O^i$ for for the $i$-th measurement by taking the median in the range \SIrange{294}{400}{\nano\meter}.
Similarly, we estimate the noise level $\eta^i$ by calculating the standard deviation in the same range.
...
We single out measurements with experimental overexposure by determining for each spectrum the maximum $M^i$ of the smoothed spectrum $m^j$.
...
\begin{equation*}
	P^i = \sqrt{\frac{1}{N_f}\sum_{j=1\ldots N_f} \qty({s}^i_j)^2}\textrm{,}
\end{equation*}
and ${s}^i_j$ is the $i$-th spectral bin of $m^j$.
Here, measurements that do not satisfy the condition $SNR^i < 2$ are classified as noisy.

For classification, we use the whole spectrum in the range \SIrange{410}{1002}{\nano\meter} (\emph{long}).
Additionally, we considered the range \SIrange{410}{680}{\nano\meter} (\emph{short}) for inspection purposes.\todo{ist das wichtig?}
Each measurement was divided by its norm $n^i$, which is particularly important for SDCM to ensure that the regression steps converge within a reasonable time.
...
For this purpose, we use the labels \textbf{Type} and \textbf{SampleID} as stratification variables.
...
\begin{itemize}
	...
	\item PCA
\end{itemize}
Here, spectra and spectra (no batchprocessing) describe methods where we do not perform any dimension reduction on the spectral data.
SDCM and PCA, on the other hand, reduce the number of dimensions in the data.
...
\textbf{For classification, to keep the number of PCs to a comparable size with the signatures in SDCM, and to achieve a reasonable dimensional reduction, we apply the following noise model: \todo{Write this once understood.}
...
\begin{figure}[h]     
	...
	\caption{\textbf{Distribution of of number of SDCM signatures (left) and number of PCs (right)}}
...
Since we want to interpret the discovered signatures, we additionally impose the restriction that for each measurement all data labels must be provided, which reduced the number of spectral data to $1243$.
Furthermore, we did not split off any validation sets.\todo{aber das hast du doch vorher auch nicht gemacht}
\subsubsection*{SDCM}
