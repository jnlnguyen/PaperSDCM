...
$\abs{\weight{k^*}{j}} \ge 0.05$. 
Using the binary variables \enquote{is part of subsignature $\ssig{k}$} and \enquote{is
part of label $l$}, we constructed a contingency table for each subsignature-label
combination, and calculate a $p$ value using a Fisher exact test. 
\todo{hypgergeometric test?}\todo{cite implementation}. We used the
Benjamini-Hochberg procedure to calculate the falls discover rate
(FDR).\todo{citaitons} 
...
subset of labels of $\labelCol{\infoCat}$, the higher $\avmems{\ssig{k}}$ is. We
define a signature to be selecting if it contains at least one label with
$\pval{\ssig{k}}{l}\le \tpval$ and $\aws{\ssig{k}}{l} \ge \taw$, and
$\avmems{\ssig{k}}\ge\tmem$. As in some specific signatures the significant
weights are either strictly positive or negative, we applied to following
correction to remove redundancies:
\begin{itemize}
    \item If $k^+, k^-, k^0$ are specific, only consider $k^0$
