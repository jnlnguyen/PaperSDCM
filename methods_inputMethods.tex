...
mean of number of bins in the overall set. \todo{Was interpolation even
necessary in the final run?}. From each measurement the associated background
measurement was subtracted. The data was concatenated into a single $x\times y$
\todo{Werte einfügen} matrix. The peak of spectrometer laser is located at
...
measurement $o_m^i$ is estimated by taking the median of the data in the spectral range
...
the same steps as in \autoref{ssub:preprocForClassification}. As we want to be
...
measurements to \todo{fill value}. Furthermore we did not split off any
validation sets.
%\subsubsection*{PCA}
%Principle Component Analysis (PCA), first introduced by \todo{insert citation},
%is a classic tool in data science used for dimensional reduction of large
%datasets. It generates a set of orthogonal vectors which diagonalize the
%covariance matrix of the data. \todo{write more}
%\todo{fill with content!}
\subsubsection*{SDCM}
...
Once a set of signatures has been found in the \emph{preTraining} set, obtaining
