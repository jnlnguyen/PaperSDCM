...
An estimation showed that in 2010 at least 4.8 million tons of plastic litter has already entered our ocean\cite{Jambeck2015a}.
Without an effective waste management plan this amount will continue to grow\cite{Jambeck2015a, Geyer2017}. 
Once in our environment, plastic can persist for decades as most types are resistant to natural degradation processes\cite{Chamas2020}.
...
Since 2021, we also have the first evidence of microplastics in our organs, when Ragusa et al. detected them in the human placenta\cite{Ragusa2021}.
...
These influences are impractical to capture and quantify as they may only apply to the examined setup.
...
An analysis of the stripped-down dataset can reveal signatures, which are essentially selection criterea, for groups of samples with common properties, such as, plastic type or color. 
The efficiency to identify and capture these signatures depends on the selected transformation algorithm.
...
This is particularly attractive for PL-based plastic litter detection as it can identify unique subspectra that only occur in specific samples or sample types.

In this report, we demonstrate two key points:
first, ML is the method of choice for PL spectroscopy based sample identification; and second, SDCM has a superior ability to identify unique sample type signatures.
The latter is advanteagous as it improves our ability to determine a single source for the sample.
To demonstrate this, we generate ML models with one of five different learning methods and one of four transformation algorithms.
By comparing model's performance metrics, e.g. the probability that a model-based prediction is correct, and the extracted signatures we find that SDCM-based ML models outperform their counterparts.
